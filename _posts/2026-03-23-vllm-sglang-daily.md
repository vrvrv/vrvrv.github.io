...
| [#37813](https://github.com/vllm-project/vllm/pull/37813) | vLLM | Open | GDN prefill 경로에서 conv output split을 Q/K/V 분리 없이 fuse — Qwen3.5-35B-A3B 기준 prefill 단계에서 rearrange 오버헤드 제거 |
...
#37813은 Qwen3.5 같은 GDN(Gated Delta Network) 모델의 prefill에서 `causal_conv1d_fn` 출력을
`torch.split()` → `.contiguous()` 로 Q/K/V를 분리하던 경로를 `fuse`하여 불필요한 메모리 복사를 제거한다.
...
Qwen3.5-397B INT4 TP=2에서 20% pruning이 정확도 손실 없이 (49/50 math) 동작함을 확인했다.
...
recurrence update와 함께 하나의 커널로 합친다. #37789는 같은 GDN의 prefill 경로에서 `causal_conv1d_fn` 출력을
`torch.split()` → `.view()` → implicit `.contiguous()`로 Q/K/V를 분리하던 3-step을 fused rearrange로 전환한다.
...
Attention KV는 모든 head가 동일 크기라 flat split이 가능하지만, Mamba conv_states는 `[x | B | C]`에서 x_dim ≠ B_dim이므로 각 D rank가 필요한 데이터가 비연속적이다. 3가지 접근이 Nemotron-Nano-30B에서 검증되었다:
