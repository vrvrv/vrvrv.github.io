...
이 세 축의 교차점은 "Hybrid Model(GDN/Mamba+Attention) + MoE + P/D"가 동시에 활성화되는 배포 시나리오이며, 이전 포스트(3/11) 이후 이 조합이 만드는 경계조건(deadlock, livelock, OOM)이 연이어 보고되면서 각 축의 변경이 가속되고 있다.
...
| [#37729](https://github.com/vllm-project/vllm/issues/37729) | vLLM | Open | V1 engine core silent deadlock — /health 200이지만 0 token 생성, fp8 + prefix caching + async scheduling + Qwen3.5 조합 |
...
#37729는 V1 엔진의 silent deadlock으로, fp8 + prefix caching + async scheduling + Qwen3.5 조합에서 concurrent load 시 /health는 200을 반환하지만 token 생성이 완전히 멈추는 심각한 이슈다. running 상태의 request가 영구 체류하며 pod 재시작 없이는 복구가 불가능하다.
